...
def build_cnn(input_var=None):
    ...
    network,stacks,layers,errors,watchpoints=deepstacks.lasagne.build_network(network,(
        ...
        # push into named stack 'source'
        (0,0,2,2,'source',0,{'maxpool'}),
        (0,0,2,0,0,0,{'upscale'}),
        (0,32,5,1,0,0,{}),
        (0,0,2,0,0,0,{'upscale'}),

        # named constraint 'recon': the output of current layer should equal to
        # the orig image comparing using 'squared_error'. see following
        # 'get_loss' for extra code needed for using this feature.
        (0,1,5,1,0,0,{'equal':['input','recon',lasagne.objectives.squared_error]}),

        # reference top item of named stack 'source' as the current layer
        ('source',),
        (dropout,0.5),
        (0,256,0,0,0,0,{'dense'}),
        (dropout,0.5),
