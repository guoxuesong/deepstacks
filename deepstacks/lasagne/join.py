...
def copylayer(layer, prefix):
    ...
    return CopyMergeLayer(layer, prefix) if isinstance(
            layer, lasagne.layers.MergeLayer) else CopyLayer(layer, prefix)
...
def copy_batch_norm(layer):
    ...
    if isinstance(real, BatchNormLayer):
        ...
        raise Exception(
                'Can not handle lasagne.layers.BatchNormLayer,'
                'you should use lasagne.layers.join.BatchNormLayer instead.')
